    ...
    <div className="max-w-6xl mx-auto p-4 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex bg-zinc-900 border border-zinc-800 rounded-xl p-1">
          <button 
            ...
            className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all ${activeTab === 'generate' ? 'bg-zinc-800 text-white shadow-lg' : 'text-zinc-500 hover:text-zinc-300'}`}
          >
            Create New
          </button>
          <button 
            onClick={() => { setActiveTab('edit'); setResultImage(null); }}
            className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all ${activeTab === 'edit' ? 'bg-zinc-800 text-white shadow-lg' : 'text-zinc-500 hover:text-zinc-300'}`}
          >
            Edit Asset
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-7">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl h-full min-h-[500px] flex flex-col shadow-2xl overflow-hidden">
            ...
            <div className="flex-1 bg-zinc-950 flex items-center justify-center p-8">
              {isProcessing ? (
                <div className="flex flex-col items-center gap-4 animate-pulse">
                  ...
                  <p className="text-sm font-bold text-blue-400 tracking-widest uppercase">Rendering Vector Space...</p>
                </div>
              ) : resultImage ? (
